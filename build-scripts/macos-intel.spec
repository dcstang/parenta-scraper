...
"""
PyInstaller spec file for macOS Intel (x86_64) builds
Specifically targets Intel chipsets for compatibility with older Mac hardware
...
exe = EXE(
    ...
    target_arch='x86_64',  # Force Intel x86_64 architecture
...
app = BUNDLE(
    ...
    # icon='assets/icon.icns',
    bundle_identifier='com.parentascraper.intel.app',
    version='1.0.0',
    info_plist={
        'CFBundleName': 'Parenta Scraper (Intel)',
        'CFBundleDisplayName': 'Parenta Scraper (Intel)', 
        'CFBundleVersion': '1.0.0',
        'CFBundleShortVersionString': '1.0.0',
        'CFBundleIdentifier': 'com.parentascraper.intel.app',
        'LSMinimumSystemVersion': '10.13.0',  # macOS High Sierra+ for Intel
        ...
        'LSRequiresNativeExecution': True,  # Ensure native execution on Intel
        'LSArchitecturePriority': ['x86_64'],  # Prefer Intel architecture
    },
)
...
2. Add README with Chrome installation instructions specifically for Intel Macs
3. Test the .app bundle on Intel hardware
4. Optional: Code sign the app for distribution

Chrome Detection on macOS Intel:
- App will look for Chrome at /Applications/Google Chrome.app
- Intel Macs can run both Intel and Apple Silicon Chrome versions
- If not found, shows friendly error with Intel-specific download link
- No Chrome bundling on Mac (not feasible)

Intel-Specific Notes:
- Built specifically for x86_64 architecture
- Compatible with Intel Macs from 2015-2020+
- Will run under Rosetta on Apple Silicon Macs if needed
- Lower system requirements (macOS 10.13+) for older Intel hardware

Directory structure after build:
dist/ParentaScraper.app/
├── Contents/
│   ├── Info.plist              # App metadata (Intel-specific)
│   ├── MacOS/
│   │   └── ParentaScraper      # Intel x86_64 executable
│   ├── Resources/              # App resources
│   │   ├── batch_extractor.py
│   │   └── ... (dependencies)  
│   └── Frameworks/             # Python framework (Intel x86_64)
"""